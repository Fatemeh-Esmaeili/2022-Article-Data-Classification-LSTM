...
%             end
        ...
        function  [Result,RefereceResult]= getValues(c_matrix)
            ...
            Result.Sensitivity=mean(Sensitivity,'omitnan');
            Result.Specificity=mean(Specificity,'omitnan');
            Result.Precision=mean(Precision,'omitnan');
            Result.FalsePositiveRate=mean(FPR,'omitnan');
            Result.F1_score=mean(F1_score,'omitnan');
            Result.MatthewsCorrelationCoefficient=mean(MCC,'omitnan');
